...
# telethon and pyrogram have different session file formats, because rm_reactions.py creates 2 sessions and you have to log in twice
...
async def main():
    async with app:
        ...
        parser.add_argument(
            ...
            "--list",
            action="store_true",
            help="list chats with reactions but do not delete",
        )
        ...
        selected_group_indexs = input(
            "Select one or many groups by entering their indexes comma separated: "
        )
...
async def rm_reactions_from_chat(app, chat_id, me, rm: bool = False):
    ...
    async for message in client.iter_messages(chat_id):
        print(
            f"Progress: {i + 1}/{numMessages} ({(i + 1) / numMessages * 100:.2f}%)",
            end="\r",
        )
